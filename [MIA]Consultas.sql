...
*/
select nombre AS Proveedor,telefono,id_venta as Numero_venta,total as Total from VENTA,PROVEEDOR where total = (select max(total) from VENTA) and cod_proveedor = id_proveedor;
#Done_well
...
*/
(select direccion,region,ciudad,codigo_postal,count(cod_ubicacion) from UBICACION,VENTA where id_ubicacion = cod_ubicacion group by cod_ubicacion having count(cod_ubicacion) = (select max(sumatoria) from (select count(cod_ubicacion) as sumatoria from VENTA group by cod_ubicacion)T))
union
...
*/
select nombre as Top_Proveedores,sum(sub_total) as Total_dinero from PROVEEDOR,VENTA,DETALLE_VENTA,PRODUCTO WHERE cod_venta = id_venta and cod_proveedor = id_proveedor and cod_producto = id_producto and cod_categoria_producto = (select id_categoria_producto from CATEGORIA_PRODUCTO WHERE categoria_producto = 'Fresh Vegetables') group by nombre order by sum(sub_total) desc limit 5;
#Done_well++
...
*/
(select direccion,region,ciudad,codigo_postal,sum(total) as Total from UBICACION,COMPRA where COMPRA.cod_ubicacion = id_ubicacion group by direccion,region,ciudad,codigo_postal,cod_cliente having sum(total) = (select max(sumatoria) from (select sum(total) as sumatoria from COMPRA group by cod_cliente)T))
union
